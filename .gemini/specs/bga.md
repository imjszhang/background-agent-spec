...
## 执行完毕后自动反馈评估机制（V1.4.0新增）

### 强制要求
每次任务执行完毕后，系统**必须**自动执行结果评估并更新相关规划的feedback.md文件：

#### 1. 质量评估体系
- **完成度分析**：统计任务完成率，分析未完成任务的原因
- **质量评分**：基于产出质量、执行效率、错误率等维度进行综合评分（1-10分）
- **标准符合性**：检查执行结果是否符合预定的质量标准和规范要求
- **用户期望对比**：将执行结果与用户初始期望进行对比分析

#### 2. 问题发现机制
- **异常检测**：自动识别执行过程中的异常情况和潜在问题
- **性能瓶颈**：分析执行时间超出预期的任务和步骤
- **资源浪费识别**：检测不必要的资源消耗和重复操作
- **依赖关系问题**：识别任务间依赖关系导致的执行效率问题

#### 3. 优化建议生成
- **流程改进**：基于执行结果提出工作流程的改进建议
- **任务分解优化**：建议更合理的任务分解和执行顺序
- **工具使用优化**：推荐更高效的工具和方法
- **并行执行机会**：识别可以并行执行的任务组合
...
**质量评分**: [1-10]/10  
**完成度**: [已完成任务数]/[总任务数] ([完成率%])  
**执行时长**: [实际用时] / [预估时间] ([效率百分比])  
**主要问题**: [发现的关键问题清单]  
**优化建议**: [具体的改进建议]  
**内容**: 执行完成质量评估结果：[详细评估分析]  
**处理结果**: 建议在下次执行时采纳相关优化措施  
```

#### 5. 多维度分析机制
- **时间效率分析**：对比预估时间与实际执行时间，识别时间估算偏差
- **错误模式分析**：统计和分类执行过程中的错误类型和频率
- **工具使用分析**：评估各种工具的使用效果和适用场景
- **并发执行分析**：评估并行任务的协调效果和资源竞争情况

#### 6. 持续改进机制
- **历史对比**：与同类规划的历史执行结果进行对比分析
- **趋势识别**：识别执行质量和效率的变化趋势
- **最佳实践提取**：从高质量执行中提取可复用的最佳实践
- **预测模型优化**：基于反馈数据优化未来任务的时间和资源预估

#### 7. 反馈触发条件
- **强制触发**：每次执行完成后无条件触发
- **质量阈值触发**：当质量评分低于预设阈值时触发详细分析
- **异常情况触发**：当检测到严重异常或失败时触发深度分析
- **用户请求触发**：用户主动请求执行结果评估时触发
